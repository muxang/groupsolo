...
namespace Miningcore.Extensions
{
    public static class ArrayExtensions
    {
        ...
        {
            return ToHexString(value.AsSpan(), null, null, withPrefix);
        }

        public static string ToHexString(this Span<byte> value, int? off = null, int? len = null, bool withPrefix = false)
        {
            if (value == null || value.Length == 0)
                return string.Empty;

            int start = off ?? 0;
            int length = len ?? value.Length;
            ...
            Span<char> buffer = bufferSize <= 128 ? stackalloc char[bufferSize] : new char[bufferSize];
            int offset = 0;
            ...
            {
                var hex = HexStringTable[value[start + i]];
                buffer[offset + i * 2] = hex[0];
